...
3. **Go Native Bridge Daemon & Embedded MCP Server** (`main.go`): Runs on port `26888`, providing `/mcp` (Streamable HTTP MCP) and `/proxy.pac` (SwitchyOmega PAC generator with LAN bypass).
...
### Workflow 3: Testing Local Services (Port 26888)
...
3. **Keep Port Unified**: Both `/mcp` and `/proxy.pac` must run strictly on port `26888`.
