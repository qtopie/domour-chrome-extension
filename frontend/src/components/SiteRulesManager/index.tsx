...
export default function SiteRulesManager({ isExtension }: SiteRulesManagerProps) {
  ...
  const removeHost = (host: string) => {
    ...
    sendMessage<any>({ type: "REMOVE_SITE_RULE", host }, (res) => {
      if (res && res.rules) setRules(res.rules);
      setMessage(t("siterules.removed", { host }));
      setTimeout(() => setMessage(null), 2000);
    });
  };
  ...
  return (
    <section className="panel-card">
      <h2 className="card-title">{t("siterules.title")}</h2>
      ...
      <div className="chat-input-row">
        <input
          ...
          placeholder={t("siterules.placeholder")}
          value={hostInput}
          onChange={(e) => setHostInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && addHost()}
        />
        <button onClick={addHost} className="chat-send-btn">{t("siterules.addSite")}</button>
      </div>

      <div className="site-rule-row site-rule-global">
        <div className="site-rule-host">{t("siterules.globalDefault")}</div>
      ...
      {hosts.length === 0 ? (
        <div className="no-logs"><span>{t("siterules.noRules")}</span></div>
      ) : (
        hosts.map((host) => {
